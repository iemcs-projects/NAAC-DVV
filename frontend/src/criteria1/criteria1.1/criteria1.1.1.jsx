...
import api from "../../api";
import { FaTrash, FaEdit, FaDownload, FaEye } from "react-icons/fa";
...
const Criteria1_1_3 = () => {
  ...
  // New state for managing uploaded files
  ...
  const fetchUploadedFiles = async (criteriaCode, session) => {
    setLoadingFiles(true);
    try {
      const response = await api.get(`/uploads/${criteriaCode}`, {
        params: { session }
      });
      ...
      // Don't show error for 404 (no files uploaded yet)
  ...
  const handleDeleteFile = async (criteriaCode, fileId) => {
    if (!window.confirm('Are you sure you want to delete this file?')) {
      return;
    }

    try {
      const response = await api.delete(`/uploads/${criteriaCode}`);
      
      if (response.data?.success) {
        // Remove the file from local state
        setUploadedFiles(prev => prev.filter(file => file.id !== fileId));
        
        // Also remove from formData.supportLinks if it exists there
        setFormData(prev => ({
          ...prev,
          supportLinks: prev.supportLinks.filter(link => link.id !== fileId)
        }));
        ...
        await fetchUploadedFiles('1.1.3', currentYear?.split("-")[0]);
      }
    } catch (error) {
  ...
  // Function to handle file upload with better integration
  const handleFileUpload = async (files) => {
    ...
    for (const file of filesArray) {
      try {
        ...
        const yearToUse = currentYear?.split("-")[0] || new Date().getFullYear().toString();
        console.log('Using year:', yearToUse);
        
        // Create FormData for file upload
        const formDataForUpload = new FormData();
        formDataForUpload.append('file', file);
        formDataForUpload.append('session', yearToUse);
        
        // Upload file using the specific endpoint
        const response = await api.post(`/uploads/1.1.3`, formDataForUpload, {
          headers: {
  ...
  useEffect(() => {
    const loadData = async () => {
      if (currentYear) {
        ...
        // Also fetch uploaded files for this year
        await fetchUploadedFiles('1.1.3', currentYear.split("-")[0]);
      }
    };
...
/>
          ...
          <div className="mt-auto bg-white border-t border-gray-200 shadow-inner py-4 px-6 flex justify-between items-center">
                 ...
                 <div className="flex items-center gap-4 mb-2">
                   ...
                   {loadingFiles && <span className="text-gray-600">Loading files...</span>}
                 ...
                 {/* Display uploaded files from API */}
                 {uploadedFiles.length > 0 && (
                   <div className="mt-4">
                     <h4 className="text-md font-medium text-gray-700 mb-2">Uploaded Files:</h4>
                     <ul className="list-disc pl-5 text-gray-700">
                       {uploadedFiles.map((file, index) => (
                         <li key={file.id || index} className="flex justify-between items-center mb-2 p-2 bg-gray-50 rounded">
                           <div className="flex items-center flex-1">
                             <FaEye className="text-blue-500 mr-2" size={16} />
                             <a
                               href={`${process.env.REACT_APP_API_URL || 'http://localhost:3000'}${file.file_url}`}
                               target="_blank"
                               rel="noopener noreferrer"
                               className="text-blue-600 underline hover:text-blue-800 flex-1"
                               title="View file"
                             >
                               {file.file_url ? file.file_url.split("/").pop() : `File ${index + 1}`}
                             </a>
                           ...
                           <div className="flex items-center gap-2 ml-4">
                             ...
                             <button
                               type="button"
                               onClick={() => handleDeleteFile('1.1.3', file.id)}
                               className="text-red-600 hover:text-red-800 !bg-white hover:bg-gray-100 p-1 rounded transition-colors duration-200"
                               title="Delete file"
                             >
                               <FaTrash size={14} />
                             </button>
                 ...
                 {/* Display files from form data (newly uploaded in current session) */}
                 {formData.supportLinks.length > 0 && (
                   <div className="mt-4">
                     <h4 className="text-md font-medium text-gray-700 mb-2">Current Session Files:</h4>
                     <ul className="list-disc pl-5 text-gray-700">
                       {formData.supportLinks.map((link, index) => (
                         <li key={link.id || index} className="flex justify-between items-center mb-1">
                           <a
                             href={`${process.env.REACT_APP_API_URL || 'http://localhost:3000'}${link.url}`}
                             target="_blank"
                             rel="noopener noreferrer"
                             ...
                             onClick={() => {
                               ...
                               setFormData(prev => {
                                 ...
                                 if (newLinks.length < prev.supportLinks.length) {
                                   setSuccess('File removed from current session!');
                                   setTimeout(() => setSuccess(''), 3000);
                                 }
                                 return {
                                   ...prev,
          ...
          {success && (
            <div className="fixed bottom-4 right-4 bg-green-500 text-white px-6 py-3 rounded-lg shadow-lg max-w-md">
              <div className="flex items-center">
                ...
                <span>{typeof success === 'string' ? success : 'Data saved successfully!'}</span>
              </div>
            </div>
          )}
