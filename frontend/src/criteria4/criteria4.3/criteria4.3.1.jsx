import { useState, useEffect, useRef } from 'react';
import Header from "../../components/header";
import Navbar from "../../components/navbar";
import Sidebar from "../../components/sidebar";
import Bottom from "../../components/bottom";
import { useNavigate } from "react-router-dom";

const Criteria4_3_1 = () => {
  const [metrics, setMetrics] = useState([
    {
      id: '4.3.1',
      description: 'Institution frequently updates its IT facilities including Wi‑Fi',
      response: '',
      wordCount: 0,
      isComplete: false,
      isExpanded: true,
      files: [],
      mandatory: false,
      lastSaved: null
    }
  ]);

  const [saving, setSaving] = useState(false);
  const [autoSaveTimestamp, setAutoSaveTimestamp] = useState(null);
  const textareaRefs = useRef({});
  const autoSaveTimerRef = useRef(null);

  const handleResponseChange = (id, value) => {
    const wordCount = value.trim() === '' ? 0 : value.trim().split(/\s+/).length;
    setMetrics(prev => prev.map(m =>
      m.id === id
        ? { ...m, response: value, wordCount, isComplete: wordCount >= 500 }
        : m
    ));

    if (autoSaveTimerRef.current) clearTimeout(autoSaveTimerRef.current);
    autoSaveTimerRef.current = setTimeout(() => saveMetric(id), 3000);
  };

  const saveMetric = id => {
    setSaving(true);
    setTimeout(() => {
      const timeString = new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
      setMetrics(prev => prev.map(m =>
        m.id === id ? { ...m, lastSaved: timeString } : m
      ));
      setSaving(false);
      setAutoSaveTimestamp(timeString);
    }, 800);
  };

  const handleFileUpload = (id, e) => {
    if (!e.target.files) return;
    const newFiles = Array.from(e.target.files).map(file => ({
      id: Math.random().toString(36).substring(2, 9),
      name: file.name,
      size: file.size,
      type: file.type,
      file
    }));
    setMetrics(prev => prev.map(m =>
      m.id === id ? { ...m, files: [...m.files, ...newFiles] } : m
    ));
    saveMetric(id);
  };

  const removeFile = (metricId, fileId) => {
    setMetrics(prev => prev.map(m =>
      m.id === metricId
        ? { ...m, files: m.files.filter(f => f.id !== fileId) }
        : m
    ));
    saveMetric(metricId);
  };

  const handleSubmit = () => {
    const incomplete = metrics.filter(m => m.mandatory && !m.isComplete);
    if (incomplete.length > 0) {
      alert(`Please complete all mandatory metrics: ${incomplete.map(m => m.id).join(', ')}`);
      return;
    }
    alert('All metrics submitted successfully!');
  };

  const saveAllAsDraft = () => {
    metrics.forEach(m => saveMetric(m.id));
    alert('All metrics saved as draft.');
  };

  useEffect(() => {
    const attachBlurListeners = () => {
      Object.entries(textareaRefs.current).forEach(([id, el]) => {
        if (el) {
          el.addEventListener('blur', () => saveMetric(id));
        }
      });
    };
    attachBlurListeners();
    return () => {
      Object.entries(textareaRefs.current).forEach(([id, el]) => {
        if (el) {
          el.removeEventListener('blur', () => saveMetric(id));
        }
      });
      if (autoSaveTimerRef.current) clearTimeout(autoSaveTimerRef.current);
    };
  }, [metrics]);
   const navigate = useNavigate()
     const goToNextPage = () => {
    navigate("/criteria4.3.2");
  };

  const goToPreviousPage = () => {
    navigate("/criteria4.2.4");
  };

  return (
    <div className="min-h-screen w-screen bg-gray-50 flex flex-col">
      <Header />
      <Navbar />
      <div className="flex flex-1">
        <Sidebar />
        <div className="flex-1 flex flex-col p-4">
          
          {/* Header */}
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-xl font-semibold text-gray-800">Criteria 4: Infrastructure and Learning Resources</h2>
            <div className="text-sm text-gray-600">
              {new Date().toLocaleDateString('en-US', {
                weekday: 'long', year: 'numeric', month: 'long', day: 'numeric'
              })}
            </div>
          </div>

          {/* Instruction Card */}
          <div className="bg-white rounded-lg shadow-md p-6 mb-6">
            <h3 className="text-blue-600 font-medium mb-2">4.3.1 Metric Information</h3>
            <p className="text-sm text-gray-700 mb-4">
              Describe IT facilities including Wi‑Fi with date and nature of update, max 500 words.
            </p>
            <h4 className="text-blue-600 font-medium mb-2">Required Documents:</h4>
            <ul className="list-disc pl-6 text-sm text-gray-700 space-y-1">
              <li>Upload additional information</li>
              <li>Paste link for additional information</li>
            </ul>
          </div>

          {/* Metric Input */}
          <div className="flex-1">
            {metrics.map(metric => (
              <div key={metric.id} className="bg-white rounded-lg shadow-md mb-6 w-full">
                <div className={`p-4 flex justify-between items-center cursor-pointer ${metric.isExpanded ? 'bg-indigo-50' : 'bg-white'}`}>
                  <div>
                    <h3 className="text-lg font-semibold text-gray-950">{metric.id}: {metric.description}</h3>
                    <div className="text-sm text-gray-500 mt-1">
                      {metric.mandatory && <span className="text-red-600">Mandatory</span>}
                      {metric.lastSaved && <span className="ml-4">Last saved at {metric.lastSaved}</span>}
                    </div>
                  </div>
                  <div className="text-sm text-gray-500">
                    {metric.wordCount}/500 words {metric.isComplete ? '✓' : ''}
                  </div>
                </div>
                {metric.isExpanded && (
                  <div className="p-4 border-t">
                    <label className="block text-sm font-medium text-gray-700 mb-2">
                      Qualitative Response <span className="text-red-500">*</span>
                    </label>
                    <textarea
                      ref={el => (textareaRefs.current[metric.id] = el)}
                      value={metric.response}
                      onChange={e => handleResponseChange(metric.id, e.target.value)}
                      placeholder="Enter your qualitative response here..."
                      className={`w-full min-h-[200px] p-4 rounded-md shadow-sm border ${metric.wordCount >= 500 ? 'border-green-300' : 'border-gray-300'}`}
                    />
                    <div className="mt-2 flex justify-between text-xs text-gray-500">
                      <span>
                        {metric.wordCount < 500
                          ? <span className="text-amber-600"><i className="fas fa-exclamation-triangle mr-1"></i>Please reach 500 words</span>
                          : <span className="text-green-600"><i className="fas fa-check-circle mr-1"></i>Minimum met</span>}
                      </span>
                      <span>
                        {saving
                          ? <span><i className="fas fa-sync-alt fa-spin mr-1"></i> Saving...</span>
                          : metric.lastSaved
                            ? <span>Saved at {metric.lastSaved}</span>
                            : null
                        }
                      </span>
                    </div>

                    {/* File Upload */}
                    <label className="block text-sm font-medium text-gray-700 mt-4">Upload Supporting Documents</label>
                    <input
                      type="file"
                      multiple
                      onChange={e => handleFileUpload(metric.id, e)}
                      className="block w-full text-sm text-gray-900 bg-gray-50 rounded-md"
                    />
                    {metric.files.length > 0 && (
                      <ul className="mt-2 list-disc pl-5 text-sm text-gray-700 space-y-1">
                        {metric.files.map(file => (
                          <li key={file.id} className="flex justify-between">
                            <span>{file.name}</span>
                            <button onClick={() => removeFile(metric.id, file.id)} className="text-red-600 hover:underline">Remove</button>
                          </li>
                        ))}
                      </ul>
                    )}
                  </div>
                )}
              </div>
            ))}
          </div>
          <div className="bg-white rounded-lg shadow-md p-6 mb-6">
            <div className="bg-blue-50 p-4 rounded-md mb-6">
            <ul className="list-disc pl-5 space-y-2 text-sm text-gray-700">
              <li>Upload  Additional information
</li>
              
<li>Link for Additional information   </li>
            </ul>
          </div>

            <label className="block text-sm font-medium text-gray-700 mb-2">Upload Documents</label>
<div className="flex items-center mb-4">
  <label className="bg-blue-600 text-white px-4 py-2 rounded-md cursor-pointer">
    <i className="fas fa-upload mr-2"></i> Choose Files
    <input type="file" className="hidden" multiple />
  </label>
  <span className="ml-3 text-gray-600">No file chosen</span>
</div>

<label className="block text-sm font-medium text-gray-700 mb-2 mt-4">Paste Link for Additional Information</label>
<input
  type="text"
  placeholder="Enter URL here"
  className="w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm text-gray-900"
/>

            
          </div>

          {/* Footer Actions
          <div className="mt-auto bg-white border-t border-gray-200 shadow-inner py-4 px-6 flex flex-col space-y-4">
            <div className="text-sm text-gray-500">
              {autoSaveTimestamp
                ? <span><i className="fas fa-save mr-1"></i>Auto-saved at {autoSaveTimestamp}</span>
                : <span>Changes will auto-save</span>}
            </div>
            <div className="flex gap-4">
              <button
                onClick={saveAllAsDraft}
                className="px-4 py-2 border border-blue-600 bg-blue-600 text-white rounded hover:bg-blue-700 shadow-sm"
              >
                <i className="fas fa-save mr-2"></i>Save as Draft
              </button>
              <button
                onClick={handleSubmit}
                className="px-4 py-2 bg-indigo-600 text-white rounded hover:bg-indigo-700 shadow-sm"
              >
                <i className="fas fa-paper-plane mr-2"></i>Submit All Metrics
              </button> */}
              {/* <button
              onClick={() => }
              className="px-4 py-2 bg-gray-200 text-gray-800 rounded hover:bg-gray-300 shadow-sm">
                Next
              </button> */}
            {/* </div>
          </div> */}
          <div className="mt-auto bg-white border-t border-gray-200 shadow-inner py-4 px-6">
                      <Bottom onNext={goToNextPage} onPrevious={goToPreviousPage} />
                    </div>
        

        </div>
      </div>
    </div>
  );
};

export default Criteria4_3_1;
