...
const Criteria4_2_4 = () => {
  ...
  const fetchResponseData = async (year) => {
    ...
    try {
      ...
      data.forEach(item => {
        if (item.slNo) {
          localStorage.setItem(`criteria4.2.4_slNo_${item.id}`, item.slNo);
        }
      });
  ...
  const fetchScore = async () => {
    ...
    const cachedScore = localStorage.getItem(`criteria424_score_${currentYear}`);
    if (cachedScore) {
      try {
        ...
        localStorage.removeItem(`criteria424_score_${currentYear}`);
      }
    }
    
    try {
      const response = await api.get(`/criteria4/score424`);
      ...
      if (response.data) {
        ...
        localStorage.setItem(
          `criteria424_score_${currentYear}`, 
  ...
  const handleCreate = async (formDataToSubmit) => {
    ...
    try {
      ...
      const payload = {
        no_of_teachers_stds: parseFloat(formDataToSubmit.no_of_teachers_stds) || 0,
        session: parseInt(yearToSend),
      };
      
      console.log('Sending request with payload:', payload);
      const response = await api.post('/criteria4/createResponse424', payload);
      ...
      if (response.status >= 200 && response.status < 300) {
        ...
        if (response.data?.data?.sl_no) {
          localStorage.setItem(
            `criteria424_${formDataToSubmit.no_of_teachers_stds}_${yearToSend}`, 
  ...
  const handleUpdate = async (formDataToSubmit) => {
    ...
    try {
      ...
      const response = await api.put(`/criteria4/updateResponse424/${entryId}`, payload);
  ...
  const validateFormData = (dataToSubmit) => {
    ...
    if (!checkNumericField(dataToSubmit.no_of_teachers_stds)) missingFields.push("Valid Number of Teachers and Students");
  ...
  const handleDelete = async (slNo, year) => {
    ...
    try {
      setSubmitting(true);
      const response = await api.delete(`/criteria4/deleteResponse424/${slNo}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const formDataWithYear = { 
        ...
        no_of_teachers_stds: formData.no_of_teachers_stds ? 
          parseFloat(formData.no_of_teachers_stds) : 0
      };
  ...
  const handleExport = async () => {
    ...
    const headers = [
      'Serial No.',
      'Number of Teachers and Students Using Library Per Day'
    ];
    ...
    link.href = url;
    link.setAttribute('download', `criteria_4.2.4_data_${new Date().toISOString().split('T')[0]}.csv`);
    ...
    <div className={`flex-1 transition-all duration-300 ${isSidebarCollapsed ? 'ml-16' : 'ml-64'} pl-6 pr-6 pt-4`}>
        ...
        <div className="flex-1 mt-6 flex flex-col p-4">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-xl font-medium text-gray-800">Criteria 4: Infrastructure and Learning Resources</h2>
            <div className="text-sm text-gray-600">4.2 Library as a Learning Resource</div>
          </div>

          <div className="bg-white p-6 rounded shadow mb-6">
            <h3 className="text-blue-600 font-semibold mb-2">4.2.4 Metric Information</h3>
            ...
            <ul className="list-disc pl-5 text-sm text-gray-700">
              ...
              <li className="mb-1">Method of computing per day usage of library</li>
              <li className="mb-1">Number of users using library through e-access</li>
              <li className="mb-1">Number of physical users accessing library</li>
            </ul>
          </div>
            ...
            {loading ? (
              ...
              <div>
                <p className="text-lg font-semibold text-green-800">
                  Provisional Score (4.2.4): {provisionalScore.data.score_sub_sub_criteria || provisionalScore.data.score || 0}%
                </p>
                <p className="text-lg font-semibold text-green-800">
          ...
          <div className="overflow-auto border rounded mb-6">
            <h2 className="text-xl font-bold bg-blue-100 text-gray-800 px-4 py-2">
              Library Usage Data - {isEditMode ? 'Edit Mode' : 'Add New'}
            </h2>
                  ...
                  <tr>
                    <td className="px-2 py-2 border">
                      <input
                        ...
                        step="1"
                        min="0"
                        value={formData.no_of_teachers_stds}
                        onChange={(e) => handleChange("no_of_teachers_stds", e.target.value)}
                        className="w-full px-2 py-1 border rounded text-gray-900"
                        placeholder="Number of users per day"
                        required
                      />
                    </td>
                    <td className="px-2 py-2 border">
          ...
          <div className="mb-6">
            <label className="block text-gray-700 font-medium mb-2">
              Upload Documents (Accession Register | Usage Records | E-access Reports)
            </label>
          ...
          {availableSessions?.map((session) => (
            <div key={session} className="mb-8 border rounded">
              ...
              {yearData[session] && yearData[session].length > 0 ? (
                <table className="w-full text-sm border border-black">
                  <thead className="bg-gray-200">
                    <tr>
                      ...
                      <th className="border border-black px-4 py-2 text-gray-800">Number of Users Per Day</th>
                      <th className="border border-black px-4 py-2 text-gray-800">Actions</th>
                    </tr>
                  </thead>
                  <tbody>
                    {yearData[session].map((entry, index) => (
                      <tr key={`${entry.no_of_teachers_stds}-${entry.year}-${index}`}>
                        <td className="border border-black !text-black px-4 py-2 text-center">{index + 1}</td>
                        <td className="border border-black text-black px-4 py-2">{entry.no_of_teachers_stds} users</td>
                            ...
                            <button
                              ...
                              onClick={() => {
                                setFormData({
                                  ...
                                  no_of_teachers_stds: entry.no_of_teachers_stds,
                                  supportLinks: [],
                                });
                                setEditKey({ 
                                  slNo: entry.sl_no || entry.slNo, 
                ...
                <p className="px-4 py-2 text-gray-500">No library usage data submitted for this year.</p>
              )}
            </div>
          ))}
              ...
              <table className="table-auto border-collapse w-full">
                ...
                <tbody>
                  <tr>
                    ...
                    {availableSessions?.slice(0, 5).map((year) => {
                      const entry = yearData[year]?.[0];
                      return (
                        <td key={year} className="border border-black px-4 py-2 text-center">
                          {entry ? `${entry.no_of_teachers_stds} users` : "-"}
                        </td>
                      );
                    })}
                  </tr>
