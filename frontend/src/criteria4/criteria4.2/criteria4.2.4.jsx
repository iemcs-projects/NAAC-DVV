...
const Criteria4_2_4 = () => {
  ...
  const fetchResponseData = async (year) => {
    ...
    try {
      ...
      data.forEach(item => {
        if (item.sl_no) {
          localStorage.setItem(`criteria4.2.4_slNo_${item.id || item.sl_no}`, item.sl_no);
        }
      });
  ...
  const handleSubmit = async (e) => {
    ...
    // Basic validation
    if (!formData.no_of_teachers_stds) {
      ...
      if (isUpdating && recordId) {
        try {
          ...
          const updatePayload = {
            ...
            no_of_teachers_stds: parseFloat(formData.no_of_teachers_stds) || 0
          };
          ...
          const endpoint = `/criteria4/updateResponse424/${recordIdNum}`;
        ...
        const endpoint = '/criteria4/createResponse424';
        ...
        if (response.status >= 200 && response.status < 300) {
          ...
          if (response.data?.data?.sl_no) {
            localStorage.setItem(
              `criteria424_${formData.no_of_teachers_stds}_${sessionYear}`, 
    ...
    <div className="flex flex-1 overflow-hidden pt-8">
      ...
      <div className={`flex-1 transition-all duration-300 overflow-y-auto ${isSidebarCollapsed ? 'ml-16' : 'ml-64'} pl-6 pr-6 `}>
        ...
        <div className="flex justify-between items-center mb-6">
          ...
          <h2 className="text-xl font-bold text-gray-500 mb-4">Library Usage Entry</h2>
