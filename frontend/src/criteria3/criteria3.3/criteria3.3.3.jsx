...
const Criteria3_3_3 = () => {
  ...
  const fetchResponseData = async (year) => {
    ...
    try {
      ...
      const response = await api.get(
        "/criteria2/getResponse/3.3.3", 
        { 
          params: { 
            session: yearToSend,
            criteriaCode: '030303030303'
          }
        }
      );
  ...
  const fetchScore = async () => {
    ...
    const cachedScore = localStorage.getItem(`criteria333_score_${currentYear}`);
    if (cachedScore) {
      try {
        ...
        localStorage.removeItem(`criteria333_score_${currentYear}`);
      }
    }
    
    try {
      const response = await api.get(`/criteria3/score333`);
      ...
      if (response.data) {
        ...
        localStorage.setItem(
          `criteria333_score_${currentYear}`, 
  ...
  const handleExport = async () => {
    ...
    link.href = url;
    link.setAttribute('download', `criteria_3.3.3_data_${new Date().toISOString().split('T')[0]}.csv`);
        ...
        <div className={`flex-1 transition-all duration-300 overflow-y-auto ${isSidebarCollapsed ? 'ml-16' : 'ml-64'} pl-6 pr-6 `}>
          ...
          <div className="bg-white p-6 rounded shadow mb-6">
            ...
            <ul className="list-disc pl-5 text-sm text-gray-700">
              ...
              <li className="mb-1">List of extension activities during last 5 years (Data Template)</li>
            </ul>
          </div>
              ...
              <table className="min-w-full border text-sm text-left">
                <thead className="bg-gray-100 font-semibold text-gray-950">
                  <tr>
                    ...
                    <th className="px-4 py-2 border">Collaborating Agency</th>
                ...
                <table className="w-full text-sm border border-black">
                  <thead className="bg-gray-200">
                    <tr>
                      ...
                      <th className="border border-black px-4 py-2 text-gray-800">Collaborating Agency</th>
                      ...
                      <th className="border border-black px-4 py-2 text-gray-800">Student Count</th>
                  ...
                  <tbody>
                    {yearData[session].map((entry, index) => (
                      <tr key={`${entry.activity_name}-${entry.year}-${index}`}>
                        ...
                        <td className="border border-black text-black px-4 py-2">{entry.student_count}</td>
