...
const Criteria3_3_2 = () => {
  ...
  const fetchScore = async () => {
    ...
    const cachedScore = localStorage.getItem(`criteria332_score_${currentYear}`);
    if (cachedScore) {
      try {
        ...
        localStorage.removeItem(`criteria332_score_${currentYear}`);
      }
    }
    
    try {
      const response = await api.get(`/criteria3/score332`);
      ...
      if (response.data) {
        ...
        localStorage.setItem(
          `criteria332_score_${currentYear}`, 
  ...
  const handleDelete = async (slNo, year) => {
    ...
    try {
      setSubmitting(true);
      const response = await api.delete(`/criteria3/deleteResponse332/${slNo}`);
  ...
  const handleExport = async () => {
    ...
    submittedData.forEach((entry, index) => {
      const row = [
        index + 1,
        `"${entry.name_of_activity || ''}"`,
        `"${entry.organizing_unit || ''}"`,
        entry.start_date || '',
        entry.end_date || '',
        entry.number_of_participants || ''
      ];
    ...
    link.href = url;
    link.setAttribute('download', `criteria_3.3.2_data_${new Date().toISOString().split('T')[0]}.csv`);
    ...
    <div className={`flex-1 transition-all duration-300 ${isSidebarCollapsed ? 'ml-16' : 'ml-64'} pl-6 pr-6 pt-4`}>
        ...
        <div className="flex-1 mt-6 flex flex-col p-4">
          <div className="flex justify-between items-center mb-4">
            ...
            <div className="text-sm text-gray-600">3.3 - Research Publications and Awards</div>
          </div>

          <div className="bg-white p-6 rounded shadow mb-6">
            <h3 className="text-blue-600 font-semibold mb-2">3.3.2 Metric Information</h3>
            ...
            <ul className="list-disc pl-5 text-sm text-gray-700">
              <li className="mb-1">Upload self-attested letter with the list of awards and recognitions</li>
              <li className="mb-1">Total number of awards/recognitions received from Government bodies during the last five years</li>
            </ul>
          </div>
            ...
            {loading ? (
              ...
              <div>
                <p className="text-lg font-semibold text-green-800">
                  Provisional Score (3.3.2): {provisionalScore.data.score_sub_sub_criteria || provisionalScore.data.score || 0}%
                </p>
                <p className="text-lg font-semibold text-green-800">
          ...
          <div className="mb-6">
            <label className="block text-gray-700 font-medium mb-2">
              Upload Documents (Award Letters | Recognition Certificates)
            </label>
          ...
          {availableSessions?.map((session) => (
            <div key={session} className="mb-8 border rounded">
              ...
              {yearData[session] && yearData[session].length > 0 ? (
                            ...
                            <button
                              ...
                              onClick={() => {
                                setFormData({
                                  ...
                                  name_of_activity: entry.name_of_activity,
                                  organizing_unit: entry.organizing_unit,
                ...
                <p className="px-4 py-2 text-gray-500">No awards and recognitions submitted for this year.</p>
              )}
            </div>
          ))}
